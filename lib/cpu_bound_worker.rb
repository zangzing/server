...
module Delayed
  class CpuBoundWorker < Worker
    ...
    def self.backend=(backend)
      if backend.is_a? Symbol
        ...
        backend = "Delayed::Backend::#{backend.to_s.classify}::CpuBoundJob".constantize
      end
      @@backend = backend
      silence_warnings { ::Delayed.const_set(:CpuBoundJob, backend) }
    end
    ...
    def start
      say "*** Starting CpuBoundJob worker #{name}"
      ...
      Delayed::CpuBoundJob.clear_locks!(name)
    end
   ...
   # Reschedule the job in the future (when a job fails).
    ...
    def reschedule(job, time = nil)
      if (job.attempts += 1) < self.class.max_attempts
        time ||= CpuBoundJob.db_time_now + (job.attempts ** 4) + 5
        job.run_at = time
        job.unlock
        job.save!
      else
        ...
        self.class.destroy_failed_jobs ? job.destroy : job.update_attributes(:failed_at => Delayed::CpuBoundJob.db_time_now)
      end
    end
  ...
  protected
    ...
    def reserve_and_run_one_job
      ...
      job = Delayed::CpuBoundJob.find_available(name, 5, self.class.max_run_time).detect do |job|
