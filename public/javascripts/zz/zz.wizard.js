    ...
    build_nav: function(obj, id, fade_in){
        ...
        $.each(obj.steps, function(i, item) {
            if (i == id && obj.numbers == 1) {
                ...
                temp += '<img src="' + assets.url_for('/images/wiz-num-'+temp_id+'-on.png') +'" class="num"> '+ item.title +'</li>';
            } else if (i == id) {
                value = temp_id;
                temp += '<li id="wizard-'+ i + '" class="tab on">'+ item.title +'</li>';
            } else if (obj.numbers == 1) {
                temp += '<li id="wizard-'+ i + '" class="tab">';
                temp += '<img src="' + assets.url_for('/images/wiz-num-'+temp_id+'.png') +'" class="num"> '+ item.title +'</li>';
            } else {
...
//            temp += '<li id="step-btn"><img id="next-step" src="/images/btn-steps-next.png" /></li>';
    ...
    set_wizard_style: function(style){
      if( style == 'edit'){
            $('div#drawer').css('background-image','url(' + assets.url_for('/images/bg-drawer-bottom-cap.png') + ')');
            $('div#cancel-drawer-btn').hide();
            zz.screen_gap =  160;
        } else {
            $('div#drawer').css('background-image','url(' + assets.url_for('/images/bg-drawer-bottom-cap-with-cancel.png') + ')');
