...
(function($){
	$.CustomSimpleFade = {
		...
		init: function(configs) {
			...
			this.backButtonText = 'Back to album';
		...
		parseConfigs: function(configs) {
			...
			// backButtonText
			if (typeof configs.backButtonText != 'undefined' && configs.backButtonText.length > 0) {
				this.backButtonText = configs.backButtonText;
			}
			// backgroundColor
		...
		loadControls: function(callStack) {
			...
			$('<span />').attr('id',this.controlBarIdentifier+'_ctrl1_text')
				...
				.html(localScope.backButtonText)
				.bind('click', function(){
					localScope.exitControl();
				})
				.appendTo('#'+this.controlBarIdentifier+'_ctrl1');
