class SmugmugPhotosController < SmugmugController
  ...
  def import
    ...
    photo = Photo.create(:state => 'new', :title => (photo_info[:caption].blank? ? photo_info[:filename] : photo_info[:caption]), :album_id => params[:album_id])
