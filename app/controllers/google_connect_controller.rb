...
class GoogleConnectController < ApplicationController
  ...
  def index
    ...
    client.authsub_token = session[:google_token]
  ...
  def authorize
    ...
    next_url = "http://#{self.request.host}:#{self.request.port}/google_connect/receive_token"
  ...
  def receive_token
    ...
    session[:google_token] = upgraded_token

    redirect_to '/google_connect/index'
  end
end
